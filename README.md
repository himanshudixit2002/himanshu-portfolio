...
- Content never depends on JavaScript. Controls that only work with script carry `needs-js` and are hidden without it (an unlayered rule, so it beats their display utilities); a control that is also content, like a skill chip or a layer row, carries `js-action` and stays as plain text. Without script the phone menu can't open, so the header shows a "Menu" link (`no-js-only`) to the footer's nav, which lists every page for everyone. The first screen's entrance is a CSS animation toward the visible state; section reveals only hide content once `RevealObserver` is running (in browsers without scroll timelines; elsewhere they are CSS tied to scroll); Scene 02 falls back to the stacked layout without JS.
...
- **Homepage touches.** The first screen's glow drifts (`drift`; the layer is oversized so its edges never show as it moves). A chat can play out as it crosses the screen (`bubble-seq` with `data-bubble`). `<Magnetic>` gives a call to action a few pixels of pull on fine pointers. Small drawn details — the café's ticking seconds (a registered integer shown through a CSS counter), Elepeia's size chip and bag press — are CSS on the art's own scroll timeline. The KV explorer types two commands the first time it's well in view, and stops the moment you touch it.
...
- **Introducing Himanshu** (homepage). The availability badge (`profile.availability`, sourced in `sources.ts`) links to contact, with a `live-dot`. Every fact on the first screen is already in the content. The HD monogram writes itself stroke by stroke (`pathLength="1"` strokes). Two SVG gotchas met on the way: Chrome makes no view timeline for an `<svg>` element (hang it on an HTML wrapper), and a gradient in bounding-box units doesn't paint a perfectly straight stroke (use `userSpaceOnUse`).
...
- **Things to find** (`src/components/explore`, `src/lib/explored.ts`). Everything here is per visitor, kept in their own browser (falling back to memory when storage is blocked) and read only after mount, so the server's HTML never depends on it.
  ...
  - **The portrait says hi.** As he comes well into view he greets you by himself: he hops, the ring whirls and a speech bubble opens with "Hi, I'm Himanshu." It stays up, so the words are there without a tap. Each tap says the next line, also announced to screen readers (the greeting isn't, since nobody asked for it). The lines are invitations, never claims. They point at the shortcuts the device has (⌘K and x with a keyboard, the search button and footer switch on touch) and at things to find: the full stop that bounces, the 404 that can be fixed.
  ...
  - **Discoveries** (`src/lib/discoveries.ts`). Small secrets, recorded per visitor by `discover(id)` in `lib/explored`. The first find shows a note under the header, and the palette lists them all, with a hint for those still to find. Add one by listing it there and calling `discover` where it happens. A hint says when a find needs a wider screen, a mouse or a keyboard. Current ones:
  ...
  - **X-ray mode** (`x` anywhere but a text field, the palette, or the footer switch). It outlines the page's building blocks and labels each with how it's built, from its `data-xray` attribute.
    - **Accuracy:** a label must stay true of the code; change it when the code changes. `Stage` takes one as `xray`.
  ...
  - **Section rail** (homepage, 1280px and up). One dot per section: an observer on a line across the middle of the screen grows the current dot, from the first section down. Without JavaScript it's a plain list of links.
